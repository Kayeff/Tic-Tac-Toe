...
function returnActivePlayer(gameTurns){
  let currPlayer = 'X';
  currPlayer = gameTurns.length > 0 && currPlayer === gameTurns[0].player ? 'O' : 'X';
  return currPlayer;
}
    ...
    <main className="w-full min-h-screen bg-matt-black text-beige flex justify-center items-center flex-col">
      <div className="max-w-[50rem] w-full h-full p-8 flex items-center justify-center flex-col space-y-2">
        <div className="w-full flex items-center justify-center cursor-default">
          <h1 className="font-Urbanist tracking-tight font-semibold text-6xl">Tic-Tac-Toe</h1>
        </div>
        <div className="w-full flex items-center justify-center p-4 space-x-6">
          <Player symbol={'X'} name={player.X} isActive={activePlayer === 'X'} onNameChange={handleNameChange} isWon={winner === player.X} />
          {/* <button onClick={switchPlayerName}>
            <RiArrowLeftRightFill size={30} />
          </button> */}
          <Player symbol={'O'} name={player.O} isActive={activePlayer === 'O'} onNameChange={handleNameChange} isWon={winner === player.O} />
        </div>
        <div className="w-full flex items-center justify-center">
          {
            (winner || isDraw) ? 
            <GameOver winner={winner} isDraw={isDraw} handleReset={handleRematch} /> : 
