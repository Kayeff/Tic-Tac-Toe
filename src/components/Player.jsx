...
export default function Player({ symbol, name, isActive, onNameChange }) {
  ...
  return (
    <div
      className={`bg-basix-brown p-4 w-full relative cursor-default ${
        isActive ? "shadow-border" : ""
      }`}
    >
      {isEditing ? (
        <input
          onChange={handleNameChange}
          className="outline-hidden bg-transparent font-Urbanist"
          value={playerName}
        />
      ) : (
        <h1 className="font-Urbanist text-xl font-medium">{playerName}</h1>
      )}
      <div>
        <button
          onClick={handleClick}
